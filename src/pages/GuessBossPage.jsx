    ...
    return (
        <div className="container" style={{}}>
            {data && (
                <>
                    {data === boss.name ? (
                        <h1 style={{ color: "green" }}>Right Guess!!</h1>
                    ) : (
...
export default function GuessBossPage() {
    ...
    const handleInput = (e) => {
        ...
        const filteredSuggestions = [...bosses].filter((option) =>
    ...
    return (
        <>
            <div className="contenedor">
                <div className="contenedorBoss">
                    {boss && (
                        <img
                            style={{ zoom: zoom }}
                            draggable="false"
                            className="imagenBoss"
                            src={"http://localhost:8080" + boss.filepath}
                            alt=""
                        />
                    )}
                </div>
                            ...
                            <ul className="suggestions-list">
                                {suggestions.map((suggestion, index) => (
                                    <li key={index} onClick={() => handleSuggestionClick(suggestion)}>
                                        <img
                                            style={{ width: "32px" }}
                                            src={"http://localhost:8080" + suggestion.filepath}
                                            alt=""
                                        />
